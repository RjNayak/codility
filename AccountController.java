...
import org.whispersystems.textsecuregcm.entities.DummyOtpResponse;
import org.whispersystems.textsecuregcm.entities.GcmRegistrationId;
import org.whispersystems.textsecuregcm.entities.OnlineStatus;
import org.whispersystems.textsecuregcm.entities.OnlineStatusResponse;
import org.whispersystems.textsecuregcm.entities.RegistrationLock;
import org.whispersystems.textsecuregcm.entities.RegistrationLockFailure;
import org.whispersystems.textsecuregcm.entities.ResponseCodeEntity;
...
import org.whispersystems.textsecuregcm.storage.OnlineLastSeenManager;
...
public class AccountController {
    ...
    // private final GCMSender gcmSender_with_old_key;
    private final APNSender apnSender;
    private final OnlineLastSeenManager onlineLastSeenManager;
    ...
    public AccountController(PendingAccountsManager pendingAccounts, AccountsManager accounts,
            ...
            SmsSender smsSenderFactory, DirectoryQueue directoryQueue, MessagesManager messagesManager,
            TurnTokenGenerator turnTokenGenerator, Map<String, Integer> testDevices, RecaptchaClient recaptchaClient,
            GCMSender gcmSender, APNSender apnSender, OnlineLastSeenManager onlineLastSeenManager) {
        ...
        this.onlineLastSeenManager = onlineLastSeenManager;
    }
    ...
    public Response getPreAuth(@PathParam("type") String pushType, @PathParam("token") String pushToken,
        ...
        StoredVerificationCode storedVerificationCode = new StoredVerificationCode(null, System.currentTimeMillis(),
                pushChallenge);
        ...
        if ("fcm".equals(pushType)) {
            try {
                gcmSender.sendMessage(new GcmMessage(pushToken, number, 0, GcmMessage.Type.CHALLENGE,
                        Optional.of(storedVerificationCode.getPushCode())));
            } catch (Exception ex1) {
                // gcmSender_with_old_key.sendMessage(new GcmMessage(pushToken, number, 0,
                // GcmMessage.Type.CHALLENGE,
                // Optional.of(storedVerificationCode.getPushCode())));
                ex1.printStackTrace();
            }

        } else if ("apn".equals(pushType)) {
            apnSender.sendMessage(new ApnMessage(pushToken, number, pushChallenge, 0, true,
                    Optional.of(storedVerificationCode.getPushCode())), null, null, null, null, null);
        } else {
    ...
    public Response createAccount(@PathParam("transport") String transport, @PathParam("number") String number,
        ...
        StoredVerificationCode storedVerificationCode = new StoredVerificationCode(
                verificationCode.getVerificationCode(), System.currentTimeMillis(),
        ...
        if (testDevices.containsKey(number)) {
            ...
            System.out.println("Verification code : " + verificationCode.getVerificationCode());
        }
    ...
    // New API implement by Kailas for dummy otp
    @Timed
    @GET
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/{transport}/code2/{number}")
    public DummyOtpResponse createAccount2(@PathParam("number") String number) throws RateLimitExceededException {
        Optional<StoredVerificationCode> veriicationCode = pendingAccounts.getCodeForNumber2(number);
        // String
        // code2=veriicationCode.map(StoredVerificationCode::getPushCode).orElse(null);
        String dummycode = veriicationCode.map(StoredVerificationCode::getCode).orElse(null);
        DummyOtpResponse response = new DummyOtpResponse();
        if (dummycode != null) {
            response.setCode(dummycode);
            response.setResponseCode("200");
        } else {
            response.setCode(null);
            response.setResponseCode("500");
        }
        System.out.println("--dummy code:--" + dummycode);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/saveOnlineStatus/")
    public void SaveOnlineStatus(@Valid OnlineStatus onlineStatus) throws RateLimitExceededException {
        try {
            onlineLastSeenManager.saveOnlineStatus(onlineStatus);
            System.out.println("\nProvided Input Number:---" + onlineStatus.getNumber());
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/getOnlineStatus/{number}")
    public ResponseCodeEntity getOnlineStatus(@PathParam("number") String number) {
        ResponseCodeEntity result = new ResponseCodeEntity();
        try {
            List<OnlineStatus> status = onlineLastSeenManager.getOnlineStatus(number);

            if (status != null && !status.isEmpty()) {
                OnlineStatusResponse response = new OnlineStatusResponse();
                response.setOnline(status.get(0).isOnline());
                response.setHidden(status.get(0).isHidden());
                response.setLastseen(status.get(0).getLastseen());
                ...
                response.setHidden(null);
                response.setLastseen(0);
                result.setResponsecode("204");
                result.setResponsemessage("No Content Found");
                result.setResponselist(response);
                return result;

            }
        } catch (java.lang.NullPointerException exception) {
            logger.error(" controller Catch Error Response code--", exception);
        }
        return null;
    }

    /*
     * //API Create For save device version
     * 
     * @Timed
     * 
     * @PUT
     ...
     * @Produces(MediaType.APPLICATION_JSON)
     * 
     * @Path("/saveDeviceVersion/") public void saveDeviceVersion(@Valid
     * OnlineStatus onlineStatus) throws RateLimitExceededException { try {
     * onlineLastSeenManager.saveDeviceVersion(onlineStatus);
     * 
        ...
        try {
            ...
            if (existingAccount.isPresent()
                ...
                Optional<ExternalServiceCredentials> credentials = existingAccount.get().getRegistrationLock()
                        .isPresent() && existingAccount.get().getRegistrationLockSalt().isPresent() ? Optional.of(null)
                                : Optional.empty();
            ...
            System.err.println("number:- " + number + "password:- " + password + "userAgent:- " + userAgent);
            System.out.println("accountName:- " + accountAttributes.getName() + "Pin:-" + accountAttributes.getPin()
                    + "RegId:-" + accountAttributes.getRegistrationId() + "AccessKey:-"
                    + accountAttributes.getUnidentifiedAccessKey());
